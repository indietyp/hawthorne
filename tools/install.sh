...
main() {
  ...
  if hash apt >/dev/null 2>&1; then
    apt update
    apt install -y --force-yes python3 python3-dev python3-pip ruby ruby-dev redis-server libmysqlclient-dev libxml2-dev libxslt1-dev libssl-dev libffi-dev git supervisor mysql-client
